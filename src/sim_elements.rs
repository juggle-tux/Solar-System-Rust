...
pub trait Simulable<T,U> where T: Float, U: Integer {
    ...
    fn get_mass(&self) -> T;
    fn simulate_step(&mut self, myid: usize, other_pos: &Vec<Sim_Data<T>>, delta_t: U, G: T);
}
...
impl<T,U> Simulable<T,U> for Planet<T> where T: Float, U: Integer + NumCast {
    ...
    fn simulate_step(&mut self, myid: usize, other_sim: &Vec<Sim_Data<T>>, delta_t: U, G: T) {
        ...
        for body in other_sim.iter() {
            ...
            let single_force = distance_vec * ( G * self.mass * body.mass / distance_vec.norm().powi(3) );
